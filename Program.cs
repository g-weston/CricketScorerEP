        ...
        static void Main(string[] args)
        {
            ...
            while (lastPlayerOne == false)
            {
                ...
                if (count > 9)
                {
                    ...
                    string lastPlayer = Console.ReadLine();
                    if (lastPlayer == "NO")
                    {
                        lastPlayerOne = false;
                    }
                ...
                while (endOfOver == false)
                {
                    ...
                    if (wicket == true)
                    {
                        if ((wayOut == "BOWLED") || (wayOut == "TIMEDOUT") || (wayOut == "STUMPED") || (wayOut == "LBW") || (wayOut == "HITTWICE"))
                        {
                            ...
                            twoPlayers[currentBowler].WicketsTaken++;
                        ...
                        if (wayOut == "CAUGHT")
                        {
                            ...
                            {
                                onePlayers[nextBatsman].Facing = true;
                                onePlayers[notFacing].Facing = false;

                            }
                            if (newBatFace == "NO")
                            {
                                onePlayers[nextBatsman].Facing = false;
                                onePlayers[notFacing].Facing = true;
                            }
                            nextBatsman++;
                            totalWickets++;
                            twoPlayers[currentBowler].WicketsTaken++;
                        }
                        if (wayOut == "RUNOUT" || wayOut == "OBSTRUCTION" || wayOut == "HANDLEDBALL")
                        {
                            Console.WriteLine("Was the FACING or NOTFACING batsman out?");
                            bool runOutFacing;
                            string outFacing = Console.ReadLine();
                            if (outFacing == "FACING")
                            {
                                runOutFacing = true;
                            }
                            else if (outFacing == "NOTFACING")
                            {
                                runOutFacing = false;
                            }
                            else
